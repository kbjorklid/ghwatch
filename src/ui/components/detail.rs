...
pub fn render_detail(f: &mut Frame, area: Rect, props: DetailProps) {
    if let Some(pr) = props.prs.get(props.selected_index) {
        let icons = Icons::new(props.config.use_nerd_fonts);
        let block = Block::default()
            .borders(Borders::ALL)
            .border_style(Style::default().fg(if props.detail_focused { props.theme.highlight_fg } else { props.theme.border }))
            .title(format!(" #{} - {} {} ", pr.number, pr.title, if props.detail_focused { "(Focused)" } else { "" }))
        ...
        detail_text.push(Line::from(vec![
            ...
            Span::styled(format!("{}{} ", icons.additions(), pr.additions), Style::default().fg(props.theme.success)),
            Span::styled(format!("{}{} ", icons.deletions(), pr.deletions), Style::default().fg(props.theme.error)),
            Span::raw(" | "),
            Span::styled("Reviewers: ", Style::default().fg(props.theme.gray)),
            Span::styled(if pr.reviewers.is_empty() { "None".to_string() } else { 
                pr.reviewers.iter().map(|r| format!("{}({})", r.login, r.status)).collect::<Vec<_>>().join(", ") 
        ...
        if !props.checks.is_empty() {
            detail_text.push(Line::from(Span::styled("CI Checks:", Style::default().fg(props.theme.title).add_modifier(Modifier::BOLD))));
            for check in props.checks {
                let color = match check.conclusion.as_deref() {
                    Some("success") => props.theme.success,
                    Some("failure") | Some("error") => props.theme.error,
                    _ => props.theme.warning,
                };
                let icon = match check.conclusion.as_deref() {
                    Some("success") => icons.check_ok(),
                    Some("failure") | Some("error") => icons.check_err(),
                    _ => "○",
                };
                detail_text.push(Line::from(vec![
                    Span::raw("  "),
        ...
        if !props.timeline.is_empty() {
            detail_text.push(Line::from(Span::styled("Activity:", Style::default().fg(props.theme.title).add_modifier(Modifier::BOLD))));
            for event in props.timeline.iter().take(10) { // Limit to 10 for now
                let mut spans = vec![
                    Span::raw("  "),
                    Span::styled(format!("{} ", event.event_type), Style::default().fg(props.theme.info)),
                    Span::styled(format!("by {} ", event.actor), Style::default().fg(props.theme.text)),
                ];
                
                if let Some(content) = &event.content {
                    let truncated = if content.len() > 100 { format!("{}...", &content[..97]) } else { content.clone() };
                    spans.push(Span::styled(format!(": {} ", truncated), Style::default().fg(props.theme.text)));
                }
        ...
        // Body
        detail_text.push(Line::from(Span::styled("Description:", Style::default().fg(props.theme.title).add_modifier(Modifier::BOLD))));
