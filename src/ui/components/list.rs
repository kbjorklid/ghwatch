...
fn render_pr_item(pr: &PullRequest, is_selected: bool, config: &AppConfig, theme: &Theme, icons: &Icons, width: u16) -> Vec<Line<'static>> {
    ...
    let line1 = Line::from(vec![
        ...
        Span::styled(review_status_str, Style::default().bg(review_status_color).fg(Color::Black).add_modifier(Modifier::BOLD)),
    ]);
    ...
    for col in &config.visible_columns {
        match col {
            ...
            crate::config::Column::Age => {
                line2_spans.push(Span::styled(format!("{} ", crate::ui::format::format_relative_time(&pr.updated_at)), bg_style.fg(theme.gray)));
            }
            ...
            crate::config::Column::Comments => {
                ...
                let unresolved_spans = if unresolved_new > 0 {
                    vec![
                        ...
                        Span::styled(format!("+{}", unresolved_new), bg_style.fg(theme.info).add_modifier(Modifier::BOLD)),
                    ]
                } else {
                ...
                let conversational_spans = if conversational_new > 0 {
                    vec![
                        ...
                        Span::styled(format!("+{}", conversational_new), bg_style.fg(theme.info).add_modifier(Modifier::BOLD)),
                    ]
                } else {
                    vec![Span::styled(conversational_old.to_string(), bg_style.fg(theme.gray))]
                };

                line2_spans.push(Span::styled(format!("{} ", icons.comment()), bg_style.fg(theme.gray)));
                line2_spans.extend(unresolved_spans);
                line2_spans.push(Span::styled(format!("/{} (", pr.total_resolvable_count), bg_style.fg(theme.gray)));
...
mod tests {
    ...
    fn test_render_pr_item_selected_has_vertical_bar() {
        ...
        // Check line 1
        assert_eq!(lines[0].spans[0].content, "┃", "Line 1 should start with vertical bar when selected");
        assert_eq!(lines[0].spans[0].style.fg, Some(Color::Yellow), "Vertical bar should be yellow");
        
        // Check line 2
        assert_eq!(lines[1].spans[0].content, "┃", "Line 2 should start with vertical bar when selected");
        assert_eq!(lines[1].spans[0].style.fg, Some(Color::Yellow), "Vertical bar should be yellow");
    }
    ...
    fn test_render_pr_item_unselected_has_no_vertical_bar() {
        ...
        assert_ne!(lines[0].spans[0].content, "┃", "Line 1 should not start with vertical bar when unselected");
    }
    ...
    fn test_render_pr_item_default_columns_no_review_status_on_second_line() {
        ...
        assert!(!line2_content.contains("Pending"), "Line 2 should not contain review status 'Pending'. Content: {}", line2_content);
    }
    ...
    fn test_render_comment_count_with_delta() {
        ...
        let config = AppConfig {
            visible_columns: vec![Column::Comments],
            ..Default::default()
        };
        let theme = Theme::dark();
        let icons = Icons::new(false);
        ...
        assert!(line2_content.contains("1+2/3 (0+1)"), "Comment string should contain delta. Got: {}", line2_content);
    }

    fn create_test_pr_minimal() -> PullRequest {
        PullRequest {
            id: "".to_string(),
            number: 0,
            title: "".to_string(),
