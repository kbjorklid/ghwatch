...
pub fn render_help(f: &mut Frame, area: Rect, theme: &Theme) {
    ...
    let text = vec![
        Line::from(vec![Span::styled("Navigation", Style::default().fg(theme.info).add_modifier(Modifier::BOLD))]),
        ...
        Line::from(""),
        Line::from(vec![Span::styled("Actions", Style::default().fg(theme.info).add_modifier(Modifier::BOLD))]),
        ...
        Line::from(""),
        Line::from(vec![Span::styled("Views", Style::default().fg(theme.info).add_modifier(Modifier::BOLD))]),
        Line::from("  S            : Open Settings screen"),
        Line::from("  A            : Open Archive view"),
        Line::from("  ?            : Open this Help overlay"),
        Line::from("  q            : Quit application"),
        Line::from(""),
        Line::from(vec![Span::styled("Archive View", Style::default().fg(theme.info).add_modifier(Modifier::BOLD))]),
        Line::from("  d            : Permanently delete selected PR"),
        Line::from(""),
        Line::from(vec![Span::styled("Settings", Style::default().fg(theme.info).add_modifier(Modifier::BOLD))]),
        Line::from("  j/k, Down/Up : Navigate settings"),
        Line::from("  Space/Enter  : Toggle boolean setting or query"),
        Line::from(""),
        Line::from(Span::styled("Tip: In narrow terminals, the layout automatically switches to vertical.", Style::default().fg(theme.warning))),
    ];

    let paragraph = Paragraph::new(text).block(block).wrap(Wrap { trim: true }).style(Style::default().fg(theme.text));
